...
// 2) RootState는 rootReducer 기반으로만 선언 ---------------- ✨ 변경(순환 참조 제거)
export type RootState = ReturnType<typeof rootReducer>;

// 3) preloadedState를 받는 makeStore 도입 -------------------- ✨ 변경
...
export function makeStore(preloadedState?: Partial<RootState>) {
  return configureStore({
    ...
    //   serializableCheck: {
    //     ignoredPaths: ["userReducer.user.createdAt"], // createdAt 안 쓰면 불필요
    //   },
    // }),
  });
}
